...
function cleanPayload(payload) {
  ...
  if (payload.kind === "analysis") {
    ...
    return {
      ...
      result: {
        ...
        advice: Array.isArray(r.advice) ? r.advice.slice(0, 5).map(x => ({
          cause: typeof x?.cause === "string" ? x.cause.slice(0, 1200) : "",
          suggestion: typeof x?.suggestion === "string" ? x.suggestion.slice(0, 1800) : ""
        })) : [],
  ...
  if (payload.kind === "diagnosis") {
    return {
      ...
      scores: Array.isArray(payload.scores) ? payload.scores.slice(0, 8).map(x => ({
        key: typeof x?.key === "string" ? x.key.slice(0, 80) : "",
        score: Number.isFinite(Number(x?.score)) ? Math.max(1, Math.min(5, Number(x.score))) : 0
      })).filter(x => x.key && x.score) : [],
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      const ttlDays = Math.max(1, Math.min(3650, Number(process.env.SHARE_TTL_DAYS || 180)));
      await sql`
        INSERT INTO corelingual_shares (token_hash, kind, title, payload, expires_at)
        VALUES (${hash}, ${safe.kind}, ${title}, ${JSON.stringify(safe)}::jsonb, NOW() + (${ttlDays} * INTERVAL '1 day'))
      `;
      return res.status(200).json({ token: rawToken, url: `${req.headers["x-forwarded-proto"] || "https"}://${req.headers.host}/share/${rawToken}` });
    }
    if (req.method === "GET") {
      ...
      const rows = await sql`
        SELECT kind, title, payload, expires_at
        FROM corelingual_shares
