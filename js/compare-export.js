...
(function(){
  ...
  async function buildCompareTemplateCanvas(){
    ...
    const src=await loadTemplate(pickTemplatePath(axisCount,differenceCount));
    const W=1024,H=1536,canvas=document.createElement('canvas');
    ...
    function wrap(str,maxW,size,weight='500',maxLines=2,min=10){
      const source=String(str??'').trim();if(!source)return{lines:[],size};
      const makeLines=fontSize=>{setFont(fontSize,weight);const lines=[];let cur='';for(const ch of [...source]){const next=cur+ch;if(ctx.measureText(next).width<=maxW||!cur)cur=next;else{lines.push(cur);cur=ch;}}if(cur)lines.push(cur);return lines;};
      let s=size,lines=makeLines(s);while(lines.length>maxLines&&s>min){s--;lines=makeLines(s);}
      if(lines.length>maxLines){lines=lines.slice(0,maxLines);setFont(s,weight);let last=lines[maxLines-1]||'';while(last.length>1&&ctx.measureText(last+'…').width>maxW)last=last.slice(0,-1);lines[maxLines-1]=last+'…';}
      return{lines,size:s};
    }
    ...
    adv.forEach((a,i)=>{
      ...
      drawCenteredLines(parts.slice(0,2),bodyX,layout.advice.bodyY,18,'500',gray,layout.advice.lineH,'left');
    });
    return canvas;
  }
  ...
  async function downloadCompareCard(){
    ...
    const overlay=document.getElementById('v73DownloadAdOverlay'),count=document.getElementById('v73DownloadAdCount');overlay?.classList.add('show');for(let n=5;n>=1;n--){if(count)count.textContent=String(n);await new Promise(r=>setTimeout(r,1000));}overlay?.classList.remove('show');
