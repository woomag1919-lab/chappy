/* CoreLingual v109 — comparison template selector + image export */
(function(){
  'use strict';
  ...
  function templateIndexFromDifferenceCount(count){
    // 0〜1個 → 01、2個 → 02、3個以上 → 03
    return count <= 1 ? 0 : count === 2 ? 1 : 2;
  }

  function pickTemplatePath(axisCount, differenceCount){
  ...
  async function buildCompareTemplateCanvas(){
    ...
    if(!data?.my || !data?.partner){
      throw new Error('比較データが取得できませんでした');
    }
    ...
    const topItems = [...document.querySelectorAll('#v21CompareCard .v82-top-item')]
      .slice(0,3)
      .map((el,i)=>({
