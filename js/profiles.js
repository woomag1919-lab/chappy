...
function syncDiagnosisForProfile(p,profile){
  ...
  try{
    ...
    console.warn("diagnosis restore failed",e)
  }
}
...
function clearNameRequired(p){
  const input=document.getElementById(p+"Name"),
        error=document.getElementById(p+"NameError");
...
function save(p,opts={}){
  ...
  const existing=a.findIndex(x=>x.name===name),
        old=existing>=0?a[existing]:null;
  ...
  const item={
    ...
    ...(p==="partner"&&old?.relationship
      ?{relationship:old.relationship}
      :{})
  };

  if(existing>=0)a[existing]=item;
  else a.push(item);
...
saveMy.onclick=()=>save("my");
savePartner.onclick=()=>save("partner");
...
(function(){
  try{
    if(!document.querySelector('script[data-corelingual-v147]')){
      ...
      s.src="/js/v147-relationship-check.js?v=1471";
      s.dataset.corelingualV147="1";
      document.body.appendChild(s)
    }
  }catch(e){
    console.warn("feature loader failed",e)
  }
})();
...
(function(){
  try{
    if(!document.querySelector('script[data-corelingual-v148]')){
      const s=document.createElement("script");
      s.src="/js/v148-relationship-ui.js?v=1481";
      s.dataset.corelingualV148="1";
      document.body.appendChild(s)
    }
  }catch(e){
    console.warn("relationship UI loader failed",e)
  }
})();


/* v149：結果画面の「自分／相手」表記などを整理 */
(function(){
  try{
    if(!document.querySelector('script[data-corelingual-v149]')){
      const s=document.createElement("script");
      s.src="/js/v149-polish.js?v=1491";
      s.dataset.corelingualV149="1";
      document.body.appendChild(s)
    }
  }catch(e){
    console.warn("v149 loader failed",e)
  }
})();
